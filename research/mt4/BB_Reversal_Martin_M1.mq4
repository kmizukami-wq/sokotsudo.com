//+------------------------------------------------------------------+
//| BB_Reversal_Martin_M1.mq4                                        |
//| BB逆張り＋押し目 × 3段マーチン EA for FXTF(MT4) - M1版           |
//| 1分足専用 / M15版と並行稼働可（MagicNumberBase=20000で別管理）   |
//+------------------------------------------------------------------+
...
//| 外部パラメータ                                                    |
//+------------------------------------------------------------------+
input int    MagicNumberBase   = 20000;   // MagicNumber base (M15版とは別 20000台)
input bool   AutoMagicPerPair  = true;    // true=auto per-pair magic (recommended)
input double RiskPercent       = 0.5;     // Risk per trade (%) - M1は頻度高いので低めに
...
input int    MaxHoldingBars    = 60;      // Max holding bars (M1: 60bars=1hour)
...
void SaveState()
{
   string filename = "BB_Martin_M1_" + Symbol() + "_" + IntegerToString(g_magicNumber) + ".dat";
...
int CheckSignals()
{
   ...
   double atr     = iATR(NULL, PERIOD_M1, 14, 1);
   double atrLong = iATR(NULL, PERIOD_M1, 100, 1);
   ...
   double open1  = iOpen(NULL, PERIOD_M1, 1);   // ローソク足方向確認用
   double rsi    = iRSI(NULL, PERIOD_M1, 10, PRICE_CLOSE, 1);

   // SMA（1回ずつ取得、IsSMAUp統合で重複呼び出し排除）
   double sma200    = iMA(NULL, PERIOD_M1, 200, 0, MODE_SMA, PRICE_CLOSE, 1);
   double sma200_p5 = iMA(NULL, PERIOD_M1, 200, 0, MODE_SMA, PRICE_CLOSE, 6);
   double sma50     = iMA(NULL, PERIOD_M1, 50, 0, MODE_SMA, PRICE_CLOSE, 1);
   double sma50_p5  = iMA(NULL, PERIOD_M1, 50, 0, MODE_SMA, PRICE_CLOSE, 6);
   ...
   double bbUp1 = iBands(NULL, PERIOD_M1, 20, 2.5, 0, PRICE_CLOSE, MODE_UPPER, 1);
   double bbLo1 = iBands(NULL, PERIOD_M1, 20, 2.5, 0, PRICE_CLOSE, MODE_LOWER, 1);
   double bbUp2 = iBands(NULL, PERIOD_M1, 20, 2.5, 0, PRICE_CLOSE, MODE_UPPER, 2);
   double bbLo2 = iBands(NULL, PERIOD_M1, 20, 2.5, 0, PRICE_CLOSE, MODE_LOWER, 2);
   ...
   // --- シグナル2: 高速BB逆張り ---
   double fbbUp = iBands(NULL, PERIOD_M1, 10, 2.0, 0, PRICE_CLOSE, MODE_UPPER, 1);
   double fbbLo = iBands(NULL, PERIOD_M1, 10, 2.0, 0, PRICE_CLOSE, MODE_LOWER, 1);
   ...
   // --- シグナル3: 押し目・戻り売り ---
   double sma20 = iMA(NULL, PERIOD_M1, 20, 0, MODE_SMA, PRICE_CLOSE, 1);
...
void ManagePosition()
{
   ...
   double atr    = iATR(NULL, PERIOD_M1, 14, 1);
   int    barsHeld = (g_entryTime > 0) ? iBarShift(NULL, PERIOD_M1, g_entryTime, false) : 0;
...
int OnInit()
{
   // チャートが15分足でなければ警告
   if(Period() != PERIOD_M1)
      Alert("Warning: This EA is designed for M1 timeframe!");
   ...
   g_lastBarTime  = iTime(NULL, PERIOD_M1, 0);
   ...
   Print(StringFormat("BB_Reversal_Martin_M1 EA initialized on %s M1 (Magic=%d)", Symbol(), g_magicNumber));
...
void OnDeinit(const int reason)
{
   SaveState();
   Comment("");
   Print("BB_Reversal_Martin_M1 EA deinitialized. Reason=", reason);
}
//+------------------------------------------------------------------+
